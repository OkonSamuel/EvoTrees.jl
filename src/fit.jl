...
function grow_evotree!(m::EvoTree, ::Type{<:Device}=CPU)
    ...
    # compute gradients
    update_grads!(cache[:∇], cache[:p], cache[:y], config.loss_type)
    # subsample rows
    cache[:nodes][1].is = subsample(cache[:is_in], cache[:is_out], cache[:mask], config.rowsample, config.rng)
    # subsample cols
    sample!(config.rng, cache[:js_], cache[:js], replace=false, ordered=true)

    # instantiate a tree then grow it
    tree = Tree{L,K}(config.max_depth)
    grow! = config.tree_type == "oblivious" ? grow_otree! : grow_tree!
    grow!(
        tree,
        config,
        cache[:nodes],
    ...
    push!(trees, tree)
    predict!(cache[:p], tree, cache[:x_bin], info[:feattypes])
    cache[:nrounds] += 1
    return nothing
end
...
function grow_tree!(
    ...
    nodes[1].gain = get_gain(nodes[1].∑, config.loss_type, config)
    ...
    while length(n_current) > 0 && depth <= config.max_depth
        ...
        if depth < config.max_depth
            ...
            @threads for n ∈ sort(n_current)
                update_gains!(nodes[n], js, config, feattypes, monotone_constraints, L)
            end
        end
        ...
        for n ∈ sort(n_current)
            if depth == config.max_depth || nodes[n].∑[end] <= config.min_weight
                ...
                if best_gain > nodes[n].gain + config.gamma
                    ...
                    nodes[n<<1].gain = get_gain(nodes[n<<1].∑, config.loss_type, config)
                    nodes[n<<1+1].gain = get_gain(nodes[n<<1+1].∑, config.loss_type, config)
